...
using DbFacadeUnitTests.Core.TestFacade;
...
namespace DbFacadeUnitTests.Core.Tests.Facade
{
    [TestClass]
    public class FacadeTests: UnitTestBase
    {
        ...
        public void SuccessfullyFetchesData()
        {
            ...
            Assert.AreNotEqual(Guid.NewGuid(), model.PublicKey);
        }
        [TestMethod]
        ...
        public void SuccessfullyFetchesMultipleDataSets()
        {
            ...
            Assert.AreEqual(2, users.DbDataTables.Count());
            var roles = users.DbDataTables.ElementAt(1).ToDbDataModelList<UserRole>();
        ...
        public void SuccessfullyHandlesTransaction()
        {
            ...
            Assert.AreEqual(0, response.DbDataTables.Count());
        ...
        public void CatchesValidationError()
        {
            ...
            if (response.HasError && response.ErrorInfo.Error is ValidationException e)
            {
                Assert.AreEqual(1, e.ValidationErrors.Count());
                string errorMessage = e.ValidationErrors.First();
                Assert.IsNotNull(errorMessage);
                Assert.AreEqual("CustomString is required.", errorMessage);
            }
            else
            {
                Assert.Fail();
        ...
        public void VerifyFetchCallAsTransaction()
        {
            ...
            if (response.HasError && response.ErrorInfo.Error is Exception e)
            {
                Assert.Fail();
            }
            else
        ...
        public void SuccessfullyFetchesAltDataAsync()
        {
            RunAsAsyc(async () =>
            {
                ...
                Assert.IsNotNull(response);
                Assert.AreEqual(1, response.Count());
                FetchDataAlt model = response.First();
                Assert.AreEqual(FetchDataEnum.Pass, model.MyEnum);
                Assert.AreEqual("test string", model.MyString);
                Assert.IsNotNull(model.PublicKey);
                Assert.AreNotEqual(Guid.NewGuid(), model.PublicKey);
        ...
        private void TestBenchmark(int count, double threshold, string text)
        {
            ...
            DomainFacade.UnitTestMethods.TestBenchmark.EnableMockMode(b => { 
                b.Add(list);
                b.ReturnValue = 0;
            });
