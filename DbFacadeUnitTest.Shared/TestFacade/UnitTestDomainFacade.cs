...
namespace DbFacadeUnitTests.TestFacade
{
    ...
    internal class UnitTestDomainFacade
    {
        ...
        public IDbResponse TestUnregisteredConnectionCall()=> UnitTestMethods.TestUnregisteredConnectionCall.Mock(1);
        ...
        public IDbResponse<FetchData> TestFetchDataWithOutput()
            => UnitTestMethods.TestFetchDataWithOutput.Mock(new ResponseData { MyString = "test string" }, 1, new Dictionary<string, object> { {"MyStringOutputParam", "output response" } });
        public IDbResponse TestTransactionWithOutput(string value)
           => UnitTestMethods.TestTransactionWithOutput.Mock(new DbParamsModel<string>(value), 10, new Dictionary<string, object> { { "MyStringOutputParam", "output response" } });
        ...
        #region Async Calls
        public async Task<IDbResponse> TestUnregisteredConnectionCallAsync() => await UnitTestMethods.TestUnregisteredConnectionCall.MockAsync(1);
        ...
        public async Task<IDbResponse<FetchData>> TestFetchDataWithOutputAsync()
            => await UnitTestMethods.TestFetchDataWithOutput.MockAsync(new ResponseData { MyString = "test string" }, 1, new Dictionary<string, object> { { "MyStringOutputParam", "output response" } });
        public async Task<IDbResponse> TestTransactionWithOutputAsync(string value)
           => await UnitTestMethods.TestTransactionWithOutput.MockAsync(new DbParamsModel<string>(value), 10, new Dictionary<string, object> { { "MyStringOutputParam", "output response" } });
